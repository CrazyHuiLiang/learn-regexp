...
describe('look-around', () => {
    ...
    */
    ...
    // 并列多个环视：他要求在当前位置，所有环视的判断都必须成功；环视结构不会改变当前位置，所以先后顺序无所谓
    test('paralleling', () => {
        // 不是999开头的数字字符串
        expect(/^(?=\d+)(?!999)/.test('1234')).toBeTruthy();
        expect(/^(?=\d+)(?!999)/.test('abcd')).toBeFalsy();
        expect(/^(?=\d+)(?!999)/.test('9991234')).toBeFalsy();
    });

    // 最后一种组合是将若干环视作为多选分支排列在多选结构中，列出的多个环视只要有一个成立，整个判断就会成功；不使用多选结构时，所有列出的环视
    // 都必须成立，整个判断才成功
    test('optional', () => {
        // 一个位置，它之后要么不是数字字符，要么是一个数字字符和一个非数字字符
        expect(/^((?!\d)|(?=\d\D))/.test('ab')).toBeTruthy();
        expect(/^((?!\d)|(?=\d\D))/.test('a4')).toBeTruthy();
        expect(/^((?!\d)|(?=\d\D))/.test('4')).toBeFalsy();
        expect(/^((?!\d)|(?=\d\D))/.test('44')).toBeFalsy();
    });
});
...
/*
    断言与反向引用之间的关系
    断言不匹配任何字符，只匹配位置
    而反向引用只引用之前的捕获分组匹配的文本，之前捕获分组中锚点表示的位置信息，在反向引用时并不会保留下来
        反向引用时，并不会保留之前捕获分组中的断言
 */
